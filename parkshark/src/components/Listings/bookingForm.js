...
export function BookingForm() {
		...
		fetch(`/users/${viewingUserId}`)
			...
			.then((userData) => {
				for (let checkId in userData.renter_bookings_id) {
					if (
						allListingBookings.includes(userData.renter_bookings_id[checkId])
					) {
		...
		if (timeDelta > 0 && arrivalTime !== "" && departureTime !== "") {
			return (
				<p>
					Price: $
					{Math.round(
						((timeDelta / 3600000.0) * hourlyPrice + Number.EPSILON) * 100
					) / 100}
				</p>
			);
		} else {
			return <p>Enter a valid set of dates to determine the price</p>;
		}
	};
	const submitBooking = () => {
		if (
			isNaN(arrivalDate.getDay()) ||
	...
	if (!hasBooked) {
		...
		return (
			<div>
				<h1>
					Thanks for booking! You have the spot from&nbsp;
					{new Intl.DateTimeFormat("en-US", dateOptions).format(
						arrivalDate
					)} at {arrivalDate.toLocaleTimeString("en-US", timeOptions)}&nbsp; to{" "}
					{new Intl.DateTimeFormat("en-US", dateOptions).format(departureDate)}{" "}
					at {departureDate.toLocaleTimeString("en-US", timeOptions)}
				</h1>
